...
contract Multisig {
    event OwnersAdded(string _msg, address[] _Owners);
    ...
    constructor(address[] memory _addresses) {
        for (uint8 i = 0; i < _addresses.length; i++) {
            ...
            emit OwnersAdded("owner's populated successfully", owners);
        }
       if (owners.length > maxOwners) revert maxOwnersLimitExceeded();
    }
