...
contract ExcelSchool {
    ...
    function addStudent(
        ...
        uint _level,
        uint8 _age
    ) public onlyAdmin returns (Student memory) {
    ...
    function addStaff(string memory _name, uint _salary)  public onlyAdmin returns(Staff memory) {
    ...
    function claimStudentId(
        ...
        studentDetails[student.wallet] = student;
        userRoles[msg.sender] = Roles.student;

        require(token.transferFrom(msg.sender,address(schoolTreasury), fee));
    ...
    function payStaff(
        ...
        require(_staff.salary > 0, "Salary can't be 0 token");
        require(
            token.balanceOf(schoolTreasury) >= _staff.salary,
        ...
        emit Events.PaidStaffEvent(_staff.id,_staff.salary);
    }
